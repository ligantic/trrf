...
class BasePage:
    ...
    def get_page_error(self):
        return self._get_element(
            (By.XPATH, '//div[@class="alert alert-danger"][1]')
        ).text
...
class LoginPage(BasePage):
    ...
    def set_username(self, username):
        return self._set_element(
            self.USERNAME_ELEMENT, username, clear_existing=True
        )

    def set_password(self, password):
        return self._set_element(
            self.PASSWORD_ELEMENT, password, clear_existing=True
        )
    ...
    def get_message(self, index):
        def _get_links(web_element):
            ...
            links = [
                ...
                for link in web_element.find_elements(By.TAG_NAME, "a")
            ]
        ...
        return {
            ...
            "subject": message_row.find_element(
                By.CLASS_NAME, "messageSubject"
            ).text,
...
class ChangeEmailAddressPage(BasePage):
    ...
    SUBMIT_BTN = (
        By.XPATH,
        '//button[text()="Initiate change of email address"]',
    )
    ...
    def get_current_request(self):
        ...
        return {
            "email": current_request.find_element(
                By.ID, "id_current_request_email"
            ).text,
            "date": current_request.find_element(
                By.ID, "id_current_request_date"
            ).text,
            "status": current_request.find_element(
                By.ID, "id_current_request_status"
            ).text,
        }
...
def get_site_links(site_url, registry_code):
    ...
    return {
        ...
        for key, resource in {
            "registration": f"{registry_code}/register",
            "login": "account/login?next=/router/",
            "logout": "logout?next=/router/",
            "mailbox": "mail/outbox",
            "mailbox_empty": "mail/outbox/empty",
        }.items()
    }

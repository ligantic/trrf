...
from registry.patients.models import LivingStates, Patient
...
class PatientQueryDataTest(RDRFTestCase):
    ...
    def test_get_facets(self):
        ...
        facets = query_patient_facets(
            self._request(), registry, ["livingStatus"]
        )
    ...
    def test_get_facets_with_filter(self):
        ...
        self._create_patient(
            registry, living_state=LivingStates.ALIVE, working_groups=[qld]
        )
        self._create_patient(
            registry, living_state=LivingStates.ALIVE, working_groups=[qld, nsw]
        )
        self._create_patient(
            registry, living_state=LivingStates.ALIVE, working_groups=[nsw]
        )
        self._create_patient(
            registry, living_state=LivingStates.ALIVE, working_groups=[nsw]
        )
        self._create_patient(
            registry, living_state=LivingStates.ALIVE, working_groups=[vic, nsw]
        )
        self._create_patient(
            registry, living_state=LivingStates.DECEASED, working_groups=[vic]
        )
        self._create_patient(
            registry, living_state=LivingStates.DECEASED, working_groups=[vic]
        )
        self._create_patient(
            registry, living_state=LivingStates.DECEASED, working_groups=[nsw]
        )
        self._create_patient(
            registry, living_state=LivingStates.DECEASED, working_groups=[qld]
        )
        ...
        facets = query_patient_facets(
            ...
            registry,
            ["workingGroups"],
            filters={"livingStatus": LivingStates.ALIVE},
        )
        self.assertEqual(
            {
                "workingGroups": [
    ...
    def test_query_patient(self):
        ...
        p1 = self._create_patient(
            registry, given_names="Kyle", family_name="Botany"
        )
        p2 = self._create_patient(
            registry, given_names="Jamie", family_name="Grey"
        )

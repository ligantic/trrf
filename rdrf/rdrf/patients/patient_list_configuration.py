...
class PatientListConfiguration:
    AVAILABLE_COLUMNS = {
        ...
        "last_updated_overall_at": {
            ...
            "permission": "patients.can_see_last_updated_at",
            "class": ColumnDateLastUpdated,
        },
        "actions": {
            "label": "Actions",
    ...
    AVAILABLE_FACETS = {
        "living_status": {
            "label": _("Living Status"),
            "permission": "patients.can_see_living_status",
            "default": None,
        },
        "working_groups": {
    ...
    def _load_config(self):
        return (
            self.registry.get_metadata_item("patient_list")
            or self.DEFAULT_CONFIGURATION
        )
    ...
    def get_facets(self):
        ...
        for facet_config_item in registry_config_facets:
            key, configured_facet = self._get_item_key_and_config(
                facet_config_item
            )
            if key in self.AVAILABLE_FACETS.keys():
                gql_key = to_camel_case(key)
                configured_facets[gql_key] = {
                    **(self.AVAILABLE_FACETS[key]),
                    **configured_facet,
                }

...
import rdrf
from rdrf.helpers.settings_helpers import get_csp, get_static_url_domain
...
BANNER_TEXT = env.get("banner_text", "")
...
SECURE_HSTS_SECONDS = env.get(
    "SECURE_HSTS_SECONDS", 3600
)  # TODO: Bump to 1 week, 1 month, 1 year
...
LANGUAGES = tuple(
    (
        (lang, dict(ALL_LANGUAGES).get(lang, lang))
        for lang in LANGUAGES_ASSOC_LIST
    )
)
...
EMAIL_SUBJECT_PREFIX = env.get(
    "email_subject_prefix", "DEV {0}".format(SCRIPT_NAME)
)

# Email Notifications
# NB. This initialises the email notification form
DEFAULT_FROM_EMAIL = env.get(
    "default_from_email", "no-reply@registryframework.net"
)
...
ANYMAIL = {
    "AMAZON_SES_CLIENT_PARAMS": {
        "aws_access_key_id": env.get(
            "aws_ses_access_key_id", env.get("aws_access_key_id", "")
        ),
        "aws_secret_access_key": env.get(
            "aws_ses_secret_access_key", env.get("aws_secret_access_key", "")
        ),
...
CSRF_COOKIE_NAME = env.get(
    "csrf_cookie_name", "csrf_{0}".format(SESSION_COOKIE_NAME)
)
...
CSRF_FAILURE_VIEW = env.get(
    "csrf_failure_view", "rdrf.views.handler_views.handler_csrf"
)
...
CSP_OBJECT_SRC = ["'none'"]
CSP_SCRIPT_SRC = get_csp(["'self'", "'unsafe-inline'"], [_CSP_STATIC_URL])
...
CSP_FORM_ACTION = get_csp(["'self'"], [_CSP_STATIC_URL])

# The maximum size in bytes that a request body may be before a
# SuspiciousOperation (RequestDataTooBig) is raised.
DATA_UPLOAD_MAX_MEMORY_SIZE = (
    env.get("data_upload_max_memory_size", 2621440) or None
)
...
BREACHED_PASSWORD_DETECTION_ENABLED = env.get(
    "breached_password_detection_enabled", False
)
BREACHED_PASSWORD_ENDPOINT = env.get("breached_password_endpoint", "")
MAX_BREACHED_PASSWORD_THRESHOLD = int(
    env.get("max_breached_password_threshold", "") or 0
)
...
ACCOUNT_AUTHENTICATED_REGISTRATION_REDIRECTS = env.get(
...
if DEBUG:
    STRONGHOLD_PUBLIC_URLS += (
        r"^%s.+$" % STATIC_URL,
        r"/mail/(outbox|send_longitudinal_followups)",
    )
...
SESSION_EXPIRY_WARNING_LEAD_TIME = env.get(
    "session_expiry_warning_lead_time", 300
)
...
ENABLE_CROWDIN_IN_CONTEXT_TRANSLATION = env.get(
    "enable_crowdin_in_context_translation", False
)
...
if ENABLE_CROWDIN_IN_CONTEXT_TRANSLATION:
    ...
    CSP_FRAME_SRC += [
        "https://cdn.crowdin.com",
        "https://eresearchqut.crowdin.com",
    ]

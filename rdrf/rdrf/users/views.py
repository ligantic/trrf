...
from rdrf.security.mixins import (
    StaffMemberRequiredMixin,
    TokenAuthenticatedMixin,
)
from rdrf.users.forms import EmailChangeForm
from rdrf.users.utils import (
    EMAIL_CHANGE_REQUEST_EXPIRY_HOURS,
    EMAIL_CHANGE_REQUEST_EXPIRY_SECONDS,
    activate_email_change_request,
    initiate_email_change_request,
)
...
class BaseEmailChangeRequest(View):
    ...
    def _render_template(self, request, user, form):
        ...
        is_expired = (
            current_request
            and not current_request.request_date >= activation_expiry_hours
        )
        ...
        return render(
            request, "user/change_email_address.html", context=context
        )
    ...
    def post(self, request, *args, **kwargs):
        form = EmailChangeForm(
            request.POST, user=self.user, current_user=request.user
        )
...
def get_user_by_user(user, request_user):
    ...
    if user.is_patient:
        ...
        patients = Patient.objects.get_by_user_and_registry(
            request_user, common_registry
        )
...
class PatientEmailChangeRequestView(
    PermissionRequiredMixin, BaseEmailChangeRequest
):
    ...
    def _redirect_response(self):
        return redirect(
            reverse(
                ...
                kwargs={"patient_id": self.user.patient.id},
            )
        )


class UserAdminEmailChangeRequestView(
    PermissionRequiredMixin, StaffMemberRequiredMixin, BaseEmailChangeRequest
):
    permission_required = ("groups.change_customuser",)

    def dispatch(self, request, *args, **kwargs):
        user_id = kwargs.pop("user_id")
        self.user = get_user_by_user(
            get_object_or_404(CustomUser, id=user_id), request.user
        )
        return super().dispatch(request, *args, **kwargs)

    def _redirect_response(self):
        return redirect(
            reverse("user_email_change", kwargs={"user_id": self.user.id})
        )
...
class ActivateEmailChangeRequestView(TokenAuthenticatedMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        pending_request = EmailChangeRequest.objects.filter(
            user=self.user, status=EmailChangeRequestStatus.PENDING
        ).first()

        context = {
            "registry": self.user.my_registry,
            "pending_request": pending_request,
        }

        return render(
            request, "user/activate_email_change_request.html", context
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user == self.user:
            auth.logout(
                request
            )  # Force logout, requiring the user to reauthenticate with their new email address

...
class EmailChangeForm(Form):
    error_messages = {
        ...
        "email_mismatch": _(
            "New email address and confirm email address do not match."
        ),
        "incorrect_password": _("Your current password is incorrect"),
        "inactive_user": _(
            "An email change request requiring activation cannot be submitted for an inactive user."
        ),
    }
    ...
    new_email2 = EmailField(
        label=_("Confirm new email"),
        max_length=254,
        widget=EmailInput(attrs={"autocomplete": "email"}),
    )
    current_password = CharField(
        label=_("Current password"), widget=PasswordInput()
    )
    user_activation_required = TypedChoiceField(
        coerce=lambda value: value == str(True),
    ...
    def __init__(self, *args, **kwargs):
        ...
        ].required = self.is_activation_optional

        self.fields[
            "current_password"
        ].required = not self._is_staff_user_updating_another_user()
    ...
    def is_activation_optional(self):
        return (
            self._is_staff_user_updating_another_user()
            and self.user.has_feature(
                RegistryFeatures.USER_EMAIL_ACTIVATION_OPTIONAL_FOR_ADMIN
            )
        )
    ...
    def clean_new_email(self):
        ...
        duplicate_users = CustomUser.objects.exclude(
            username__iexact=self.user.username
        ...
        if self.user.patient:
            duplicate_patients = Patient.objects.exclude(
                email__iexact=self.user.patient.email
    ...
    def clean_current_password(self):
        ...
        if self.fields[
            "current_password"
        ].required and not self.current_user.check_password(password):
    ...
    def clean(self):
        if (
            ...
            and self.cleaned_data["user_activation_required"]
        ):

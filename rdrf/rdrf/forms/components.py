...
class RDRFContextLauncherComponent(RDRFComponent):
    ...
    def __init__(
        ...
        self.form_titles = FormTitleHelper(
            self.registry_model, current_form_name
        ).all_titles_for_user(self.user)
    ...
    def _is_consent_locked(self):
        ...
        if (
            has_config
            and self.registry_model.consent_configuration.consent_locked
        ):
            return not consent_status_for_patient(
                self.registry_model.code, self.patient_model
            )
        else:
            return False
    ...
    def _get_family_linkage_link(self):
        ...
        if pk is not None:
            return reverse(
                "family_linkage", args=(self.registry_model.code, pk)
            )
        else:
            return None
    ...
    def _get_actions(self):
        ...
        patient_context_menu = PatientContextMenu(
            self.user, self.registry_model, None, self.patient_model
        )
    ...
    def _get_multiple_contexts(self):
        ...
        def _form(context_form_group):
            ...
            if link_pair and can_view and applicable:
                ...
                (
                    form.existing_links,
                    form.existing_links_index,
                    form.existing_links_len,
                ) = self._get_existing_links(context_form_group)
                form.list_link = self._get_form_list_link(form)
                return form

        cfg_qs = (
            ContextFormGroup.objects.select_related("registry")
    ...
    def _get_existing_links(self, context_form_group, slice_len=5):
        ...
        is_current_form = (
            ...
            and context_form_group.items.filter(
                registry_form=self.registry_form
            ).exists()
        )
        current_context_id = (
            self.current_rdrf_context_model.pk if is_current_form else None
        )

        forms = self.patient_model.get_forms_by_group(
            context_form_group, self.user
        )
        ...
        for index, (context_id, url, text) in enumerate(forms):
            ...
            if (
                (index - current_index) > (slice_len / 2)
                and len(links) == slice_len
                and index_found
            ):
                break
    ...
    def _get_fixed_contexts(self):
        ...
        if registry_type == RegistryType.NORMAL:
            ...
            for form_link in self._get_normal_form_links():
                form = _Form(
                    form_link.url, form_link.text, current=form_link.selected
                )
...
class FormsButton(RDRFComponent):
    ...
    def _get_form_link_wrappers(self):
        if self.context_form_group is None:
            ...
            assert len(contexts) == 1, (
                "There should only be one context in %s"
                % self.context_form_group
            )
            ...
            context_models = self.patient_model.get_multiple_contexts(
                self.context_form_group
            )
    ...
    def _get_add_link(self):
        if (
            self.context_form_group
            and self.context_form_group.context_type == "M"
        ):
            link_url, link_text = self.context_form_group.get_add_action(
                self.patient_model
            )
...
class FormGroupButton(RDRFComponent):
    ...
    def _get_template_data(self):
        ...
        return {
            "button_caption": self.button_caption,
...
class FamilyLinkagePanel(RDRFComponent):
    ...
    def __init__(self, user, registry_model, patient_model):
        ...
        if not self.is_index:
            ...
            self.link_allowed = user.can_view_patient_link(
                patient_model.my_index
            )
            ...
            self.index_working_groups = ",".join(
                sorted(
                    [
                        wg.name
                        for wg in self.patient_model.my_index.working_groups.all()
                    ]
                )
            )
        else:

        ...
        if self.data_type:
            if self.data_type.lower() == INTEGER_TYPE:
                value = (
                    ...
                    if self.value not in ["set", "unset"]
                    else None
                )
                existing_value = (
                    parse_int(self.existing_value)
                ...
                existing_value = (
                    parse_date(self.existing_value)
                    if self.existing_value
                    else None
                )
            if not all([value, existing_value]):
                ...
                return False
        return self.eval_dict.get(self.operator, self.default_comparison)(
...
class CodeEvaluator:
    ...
    def check_condition(self, condition):
        ...
        evaluator = ConditionEvaluator(
            operator, value, final_value, condition.cde.get_data_type()
        )
        return evaluator.evaluate()

    def hidden_cdes(self, target, action, check_result):
        result = []
        if (action == HIDDEN and check_result) or (
            action == VISIBLE and not check_result
        ):
            if target.has_qualifier:
                section_cdes = [
                    self.cde_helper.get_cdes_for_section(s.cde)
                    for s in target.target_cdes
                ]
    ...
    def hidden_cdes_for_conditions(self, target, action, conditions):
        ...
        if len(conditions) > 1:
            ...
            boolean_ops = [
                op.operator for op in conditions if isinstance(op, BooleanOp)
            ]
            condition_checks = [
                self.check_condition(c) for c in conditions_only
            ]

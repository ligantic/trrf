...
class RdrfRegistrationView(RegistrationView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.registration_class = self.load_registration_class(
            request, self.form
        )
    ...
    def is_form_valid(self):
        if not self.is_recaptcha_valid():
            self.form.add_error(
                None,
                _(
                    "Please complete the I'm not a robot reCAPTCHA validation and try to Submit again"
                ),
            )
            return False
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            try:
                ...
                logger.debug(
                    "RdrfRegistrationView form_valid - new_user registered"
                )
                ...
                logger.exception(
                    "Unhandled error in registration for user %s", username
                )
        ...
        try:
            ...
            logger.debug(
                "RdrfRegistrationView post - redirecting to success url %s"
                % success_url
            )
...
class EmbeddedRegistrationView(RdrfRegistrationView):
    ...
    def load_registration_class(self, request, form):
        if hasattr(settings, "REGISTRATION_CLASS_EMBEDDED"):
            registration_class = import_string(
                settings.REGISTRATION_CLASS_EMBEDDED
            )
            return registration_class(request, form)

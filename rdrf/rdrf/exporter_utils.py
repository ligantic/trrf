...
def write_export_zip(exports, export_type):
    ...
    with zipfile.ZipFile(
        ...
        for export_data, registry in exports:
            ...
            archive.writestr(
                f"{registry.code}{partial_descriptor_str(export_type)}.yaml",
                export_data,
            )
    ...
    name = (
        ...
        + export_time
        + "_"
        + reduce(lambda x, y: x + "_and_" + y, [r.code for r in registries])
        + ".zip"
    )
...
def _export_cascading_form_definition(
    registry, context_form_groups=None, forms=None, sections=None, cdes=None
):
    non_null_args = [
    ...
    if context_form_groups:
        ...
        forms = set(form for cfg in context_form_groups for form in cfg.forms)

    if forms:
        export_definition["forms"] = forms
        sections = set(
            section for form in forms for section in form.section_models
        )

    if sections:
        export_definition["sections"] = sections
        cdes = set(cde for section in sections for cde in section.cde_models)
...
def export_context_form_groups(context_form_groups):
    ...
    for registry, context_form_groups in _group_objs_by_registry(
        context_form_groups
    ):
        export_data = _export_cascading_form_definition(
            registry, context_form_groups=context_form_groups
        )
        exports.append((export_data, registry))
...
def export_registry_dashboards(dashboards):
    ...
    for registry, dashboards in _group_objs_by_registry(dashboards):
        export_data = Exporter(registry_model=registry).partial_export(
            {"registry_dashboards": dashboards}
        )
        exports.append((export_data, registry))

...
StorageFileInfo = namedtuple(
    "StorageFileInfo",
    "item filename uploaded_by patient mime_type",
...
class ActionLoggingFileStorage:
    ...
    def uploads(self):
        return [
            ...
            if action_type == FileStorageEventType.UPLOADED
        ]
    ...
    def log_upload(self, upload):
        self.actions.append(
            FileStorageEvent(
                FileStorageEventType.UPLOADED, self.upload_to_cde_file(upload)
            )
        )

    def store_file_by_key(
        self, registry_code, patient_record, uploaded_by, key, value
    ):
        upload = store_file_by_key(
            registry_code, patient_record, uploaded_by, key, value
        )
    ...
    def delete_file(self, file_ref):
        ...
        self.actions.append(
            FileStorageEvent(FileStorageEventType.DELETED, cde_file)
        )
...
def delete_file_wrapper(file_ref):
    ...
    if django_file_id is not None:
        try:
            ...
            logger.warning(
                "Tried to delete CDEFile id=%s which doesn't exist"
                % django_file_id
            )
        except Exception:
...
def store_file(
    ...
    return {"django_file_id": cde_file.id, "file_name": original_filename}

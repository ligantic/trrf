...
def migrate_cdes_clinicaldata():
    ...
    cds = ClinicalData.objects.filter(
        collection="cdes", registry_code="fh", django_model="Patient"
    )
    ...
    try:
        with transaction.atomic():
            ...
            for cd in cds:
                ...
                if structure_valid(
                    cde_codes, source_section_code, target_section_code, cd.data
                ):
                    print(
                        " Structure validated and now transforming clinicaldata dictionary......"
                    )
                    cd.data = transform_cd_dict(
                        cde_codes,
                        source_section_code,
                        target_section_code,
                        cd.data,
        ...
        print(
            "******* Rolling back......CDE migration FAILED with an exception: %s *******"
            % ex
        )

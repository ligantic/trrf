...
class ClinicalDataCsvUtil:
    ...
    def __form_header_parts(
        self, cfg, cfg_heading, cfg_i, form, form_label_text
    ):
        ...
        with connections["clinical"].cursor() as cursor:
            cursor.execute(
                sql,
                {
                    ...
                    "cde_delim": settings.FORM_SECTION_DELIMITER,
                    "cde_keys": cde_keys,
                },
            )
            ...
            summary = {
                form_name: {
                    section_code: {
                        "count": section_count,
                        "cdes": {row[3]: {"count": row[4]} for row in cdes},
                    }
                    for (
                        section_code,
                        section_count,
                    ), cdes in itertools.groupby(
                        sections, lambda x: (x[1], x[2])
                    )
                }
                for form_name, sections in itertools.groupby(
                    rows, lambda x: x[0]
                )
            }

        # Fill in the blanks with defaults
        for form_name, section_code, cde_code in map(
            get_form_section_code, cde_keys
        ):
            # Check if any combination of this form, section, cde is not in the existing summary
            if not (
                form_name in summary
                and section_code in summary[form_name]
                and cde_code in summary[form_name][section_code]["cdes"]
            ):
                # Add defaults for for the data where it does not exist
                section_data = summary.setdefault(form_name, {}).setdefault(
                    section_code, {"count": 1, "cdes": {}}
                )
    ...
    def _form_section_cde_sort_order(self, cde_keys):
        ...
        for cde_i, cde_key in enumerate(cde_keys):
            ...
            sort_order_lookup.setdefault(
                form_name, {"order": len(sort_order_lookup), "sections": {}}
            )
            section_lookup = sort_order_lookup[form_name].get("sections")
            section_lookup.setdefault(
                section_code, {"order": len(section_lookup), "cdes": {}}
            )
            cde_lookup = sort_order_lookup[form_name]["sections"][
                section_code
            ].get("cdes")
    ...
    def __sorted_forms(self, unsorted_forms_dict, sort_order_lookup):
        return dict(
            sorted(
                unsorted_forms_dict.items(),
                key=lambda item: sort_order_lookup[item[0]]["order"],
            )
        ).items()

    def __sorted_sections(
        self, unsorted_sections_dict, sort_order_lookup, form_name
    ):
        return dict(
            sorted(
                unsorted_sections_dict.items(),
                key=lambda item: sort_order_lookup[form_name]["sections"][
                    item[0]
                ]["order"],
            )
        ).items()
    ...
    def __sorted_cdes(
        self, unsorted_cdes_dict, sort_order_lookup, form_name, section_code
    ):
        return dict(
            sorted(
                unsorted_cdes_dict.items(),
                key=lambda item: sort_order_lookup[form_name]["sections"][
                    section_code
                ]["cdes"][item[0]],
            )
    ...
    def __get_cfg_data(self, cfg, cfgs_lookup, heading_format):
        ...
        if not cfg_lookup:
            cfg_lookup = {
                "count": self.__get_max_count_cfg_entries(cfg),
    ...
    def __get_report_cfgs(self, report_design):
        report_cfg_ids = report_design.reportclinicaldatafield_set.distinct(
            "context_form_group"
        ).values("context_form_group")
        return ContextFormGroup.objects.filter(id__in=report_cfg_ids)

    def csv_headers(self, user, report_design):
        cde_keys = list(
            report_design.reportclinicaldatafield_set.order_by(
                "id"
            ).values_list("cde_key", flat=True)
        )

        # Step 1 - Summarise all clinical data
        patient_filters = PatientFilterType()
        patient_filters.working_groups = [
            wg.id for wg in report_design.filter_working_groups.all()
        ]
        patient_filters.consent_questions = [
            cq.id for cq in report_design.filter_consents.all()
        ]

        patient_ids = list(
            list_patients_query(
                user, report_design.registry, patient_filters
        ...
        for form_name, form_data in self.__sorted_forms(
            cd_summary, sort_order_lookup
        ):
            form = RegistryForm.objects.get(name=form_name)
            form_heading = self.__form_heading(
                form, report_design.cde_heading_format
            )
            for cfg in report_cfgs.filter(items__registry_form=form):
                cfg_heading, longitudinal_entries_cnt = self.__get_cfg_data(
                    cfg, cfgs_lookup, report_design.cde_heading_format
                )

                for cfg_i in range(longitudinal_entries_cnt):
                    form_key_prefix, form_label_prefix = (
                        self.__form_header_parts(
                            cfg, cfg_heading, cfg_i, form, form_heading
                        )
                    )
                    if cfg.is_multiple:
                        headers[f"{form_key_prefix}_key"] = (
                            f"{form_label_prefix}_Name"
                        )
                    ...
                    if report_design.cde_include_form_timestamp:
                        headers[f"{form_key_prefix}_meta_lastUpdated"] = (
                            f"{form_label_prefix}_Last Updated"
                        )

                    for section_code, section_data in self.__sorted_sections(
                        form_data, sort_order_lookup, form_name
                    ):
                        section = Section.objects.get(code=section_code)
                        section_heading = self.__section_heading(
                            section, report_design.cde_heading_format
                        )
                        for section_i in range(section_data["count"]):
                            for cde_code, cde_data in self.__sorted_cdes(
                                section_data["cdes"],
                                sort_order_lookup,
                                form_name,
                                section_code,
                            ):
                                cde = CommonDataElement.objects.get(
                                    code=cde_code
                                )
                                cde_heading = self.__cde_heading(
                                    cde, report_design.cde_heading_format
                                )
                                cde_key, cde_label = self.__cde_header_parts(
                                    form_key_prefix,
                                    form_key_suffix,
                                    form_label_prefix,
                                    section,
                                ...
                                if cde_data["count"] > 1 or cde.allow_multiple:
                                    for cde_i in range(cde_data["count"]):
                                        headers.update(
                                            {
                                                f"{cde_key}_{cde_i}": f"{cde_label}_{cde_i + 1}"
                                            }
                                        )
                                else:

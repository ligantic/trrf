...
class ReportDesignManager(models.Manager):
    def reports_for_user(self, user):
        ...
        return (
            ...
            .get_queryset()
            .filter(
                registry__in=registries, access_groups__in=user.get_groups()
            )
        )
...
class ReportDesign(models.Model):
    ...
    filter_working_groups = models.ManyToManyField(
        WorkingGroup, related_name="filter_working_groups", blank=True
    )
    filter_consents = models.ManyToManyField(ConsentQuestion, blank=True)
    cde_heading_format = models.CharField(
        max_length=30,
        choices=CDE_HEADING_FORMATS,
        default=ReportCdeHeadingFormat.LABEL.value,
    )
    ...
    class Meta:
        ...
        constraints = [
            UniqueConstraint(
                fields=["registry", "title"], name="unique_report_title"
            )
        ]

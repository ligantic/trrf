...
class ReportDownloadAccessCheckMixin:
    def dispatch(self, request, *args, **kwargs):
        if (
            not ReportDesign.objects.reports_for_user(request.user)
            .filter(pk=kwargs["report_id"])
            .exists()
        ):
...
class ReportDownloadView(ReportDownloadAccessCheckMixin, View):
    def get(self, request, report_id, format):
        ...
        if not is_valid:
            logger.error(errors)
            return render(
                request, "report_download_errors.html", {"errors": errors}
            )
...
class ReportDesignView(SuperuserRequiredMixin, View):
    def get(self, request, report_id=None):
        ...
        params.update(
            {
                "registry__cfgs": {
                    registry: ContextFormGroup.objects.filter(
                        registry=registry
    ...
    def post(self, request, report_id=None):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Report "{form.instance.title}" has been saved successfully.',
            )
            return redirect("report:reports_list")
        else:
            return render(
                request,
                "report_designer.html",
                _get_default_params(request, form),
            )

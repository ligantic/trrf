...
def to_snake_case(name):
    from graphene.utils.str_converters import (
        to_snake_case as to_snake_case_base,
    )
...
def to_camel_case(name):
    from graphene.utils.str_converters import (
        to_camel_case as to_camel_case_base,
    )

    camel_case_name = to_camel_case_base(
        name
    )  # Replace single underscores with camel cased name
    dot_case_name = re.sub(
        r"\_", ".", camel_case_name
    )  # Replace any remaining underscores with dots
    lower_dot_case_name = re.sub(
        r"\.([A-Z][a-z]*)?", lambda m: m.group(0).lower(), dot_case_name
    )  # Lower case the first letter following the dot
...
def validate_fields(fields, valid_fields, label):
    ...
    if len(invalid_fields) > 0:
        raise PublicGraphQLError(
            _(
                f"Invalid {label}(s) provided: {', '.join(invalid_fields)}.\n "
...
class WorkingGroupTypeFilterType(InputObjectType):
    name = graphene.String()


class DataSummaryItemSummaryType(ObjectType):
    maxCount = graphene.Int()


def create_dynamic_data_summary_type(registry):
    def resolve_model_max_count(parent, model):
        return (
            parent.all_patients.annotate(Count(model))
            .aggregate(Max(f"{model}__count"))
            .get(f"{model}__count__max")
            or 0
        )
    ...
    def resolve_max_clinician_count(parent: QueryResult, _info):
        return {
            "maxCount": resolve_model_max_count(parent, "registered_clinicians")
        }

    def resolve_max_parent_guardian_count(parent: QueryResult, _info):
        return {"maxCount": resolve_model_max_count(parent, "parentguardian")}

    def resolve_consent_question_code_values(parent: QueryResult, _info):
        return (
            ConsentQuestion.objects.filter(section__registry=parent.registry)
            .order_by("position")
            .values_list("section__code", "code")
        )

    def resolve_working_group_type_summary(
        parent: QueryResult, _info, working_group_type
    ):
    ...
    def create_working_group_types_summary():
        def resolve_values(parent, _info):
            _result, all_working_group_types = parent
            return [all_wg_type.name for all_wg_type in all_working_group_types]
            ...
            wg_type_fields.update(
                {
                    ...
                    f"resolve_{field_name}": partial(
                        resolve_working_group_type_summary,
                        working_group_type=wg_type,
                    ),
                }
            )
        return type(
            f"DynamicDataSummary_{registry.code}_working_group_types",
            (ObjectType,),
            wg_type_fields,
        )
    ...
    fields = {
        "patientaddressSet": graphene.Field(DataSummaryItemSummaryType),
        "resolve_patientaddressSet": resolve_address_summary,
        "working_groups": graphene.Field(DataSummaryItemSummaryType),
        "resolve_working_groups": resolve_working_groups,
        "registered_clinicians": graphene.Field(DataSummaryItemSummaryType),
        "resolve_registered_clinicians": resolve_max_clinician_count,
        "parentguardianSet": graphene.Field(DataSummaryItemSummaryType),
        "resolve_parentguardianSet": resolve_max_parent_guardian_count,
        "consent_question_codes": graphene.Field(
            type(
                "DynamicDataSummary_consentQuestionCodes",
                (ObjectType,),
                {
                    "values": graphene.List(graphene.List(graphene.String)),
                    "resolve_values": resolve_consent_question_code_values,
                },
            )
        ),
...
class RegisteredClinician(DjangoObjectType):
    ...
    def resolve_working_groups(clinician, _info):
        return ",".join(
            [wg.display_name for wg in clinician.working_groups.all()]
        )
...
class RegisteredUserType(ObjectType):
    ...
    def resolve_last_login(usertype, _info):
        if usertype.user:
            return (
                LoginLog.objects.filter(username=usertype.user.username)
...
class ParentGuardianType(DjangoObjectType, RegisteredUserType):
    ...
    def resolve_self_patient_id(parent_guardian, _info):
        return (
            parent_guardian.self_patient.id
            if parent_guardian.self_patient_id
            else None
        )

    def resolve_gender(parent_guardian, _info):
        return dict(ParentGuardian.GENDER_CHOICES).get(
            parent_guardian.gender, parent_guardian.gender
        )

    def resolve_preferred_language(parent_guardian, _info):
        return (
            parent_guardian.user.preferred_language
            if parent_guardian.user
            else None
        )
...
class EmailPreferencesType(ObjectType):
    ...
    def resolve_unsubscribe_all(email_preference, _info):
        return (
            ...
            else email_preference.unsubscribe_all
        )
...
class FormMetaType(ObjectType):
    last_updated = graphene.DateTime(
        description="Timestamp this form was last updated"
    )
    ...
    for cde in section_cdes:
        ...
        if cde.allow_multiple:
            fields[field_name] = graphene.List(
                graphene.String, description=cde.name
            )
        else:
...
def get_form_fields(form_key, section_models, section_cdes):
    ...
    for section in section_models:
        ...
        if section.allow_multiple:
            fields[field_name] = graphene.List(
                section_type, description=section.display_name
            )
        else:
            fields[field_name] = graphene.Field(
                section_type, description=section.display_name
            )
        fields[f"resolve_{field_name}"] = partial(
            section_resolver, section_model=section
        )
    return fields
    ...
    if cfg_model.is_fixed:
        ...
        def longitudinal_form_resolver(parent, _info, form_model):
            ...
            for clinical_datum in clinical_data:
                for form_data in clinical_datum.data["forms"]:
                    if form_data["name"] == form_model.name:
                        context_model = context_lookup.get(
                            clinical_datum.context_id
                        )
                        assert context_model, "Unknown longitudinal context"

                        form_data_list.append(
                            {
                                "key": cfg_model.get_name_from_cde(
                                    patient, context_model
                                ),
                                "meta": (clinical_datum, form_model.name),
                                "data": form_data,
                            }
                ...
                (graphene.ObjectType,),
                {
                    ...
                    "data": graphene.Field(
                        type(
                            ...
                            (graphene.ObjectType,),
                            get_form_fields(
                                form_key, section_models, section_cdes
                            ),
                        ),
                        description=form.name,
                    ),
        ...
        fields[f"resolve_{field_name}"] = partial(
            longitudinal_form_resolver, form_model=form
        )
...
def get_clinical_data_fields(registry):
    ...
    for cfg in ContextFormGroup.objects.filter(registry=registry).all():
        ...
        def cfg_resolver(parent, _info, cfg_model):
            ...
            contexts = RDRFContext.objects.filter(
                object_id=patient.id, context_form_group=cfg_model.id
            )
            ...
            # TODO: better query
            cfg_clinical_data = clinical_data.filter(
                context_id__in=[context.id for context in contexts]
            )
        ...
        if forms:
            fields[field_name] = graphene.Field(
                type(f"DynamicCFG_{cfg_key}", (graphene.ObjectType,), forms),
                description=cfg.name,
            )
            fields[f"resolve_{field_name}"] = partial(
                cfg_resolver, cfg_model=cfg
            )
    return fields
...
def get_patient_fields():
    return {
        ...
        "sex": graphene.String(),
        "resolve_sex": lambda patient, _info: dict(Patient.SEX_CHOICES).get(
            patient.sex, patient.sex
        ),
        "age": graphene.Int(),
        "resolve_age": lambda patient, _info: patient.age,
        "living_status": graphene.String(),
        "resolve_living_status": lambda patient, _info: dict(
        ...
        "resolve_working_groups": lambda patient, _info: patient_working_groups(
            patient
        ),
    }
...
def get_consent_question_fields(consent_section):
    ...
    for consent_question in consent_section.questions.all():
        ...
        consent_fields[f"resolve_{field_name}"] = partial(
            consent_question_resolver, consent_question=consent_question
        )
...
def get_consent_section_fields(registry):
    ...
    for consent_section in registry.consent_sections.all():
        ...
        consent_fields[field_name] = graphene.Field(
            type(
                f"DynamicConsentSection_{consent_section.registry.code}_{consent_section.code}",
                (graphene.ObjectType,),
...
def patient_working_groups(patient):
...
def create_dynamic_facet_type(registry):
    def resolve_facet(parent, _info, facet_field, get_label_fn):
        results = (
            parent.all_patients.values(facet_field)
    ...
    def get_living_status_label(status_id):
        living_states_dict = {
            ...
            for choice_id, choice_label in LivingStates.CHOICES
        }
    ...
    available_facets = [
        ("living_status", get_living_status_label),
    ...
    for facet in available_facets:
        ...
        facet_fields[f"resolve_{field}"] = partial(
            resolve_facet, facet_field=field, get_label_fn=get_label
        )
...
def create_dynamic_patient_type(registry):
    def consent_values_resolver(patient, _info):
        return patient, ConsentValue.objects.filter(patient=patient)

    def working_group_type_values_resolver(patient, _info):
        return patient, patient.working_groups.all()

    def clinical_data_resolver(patient, _info):
        return patient, ClinicalData.objects.filter(
            django_id=patient.id, django_model="Patient", collection="cdes"
    ...
    def parent_email_preferences_resolver(patient, _info):
        ...
        return (
            None
            if parent is None
            else EmailPreference.objects.get_by_user(parent.user)
        )

    schema_module = import_module(settings.SCHEMA_MODULE)
    patient_fields_func = getattr(
        schema_module, settings.SCHEMA_METHOD_PATIENT_FIELDS
    )
    ...
    working_group_types = create_working_group_types_fields(registry)
    if working_group_types:
        patient_fields.update(
            {
                "working_group_types": graphene.Field(
                    type(
                        f"DynamicWorkingGroupType_{registry.code}",
                        (graphene.ObjectType,),
                        working_group_types,
                    ),
                ),
                "resolve_working_group_types": working_group_type_values_resolver,
            }
        )
    ...
    return type(
        f"DynamicPatient_{registry.code}", (DjangoObjectType,), patient_fields
    )


def list_patients_query(user, registry, filter_args=None):
    patient_ids = Patient.objects.get_by_user_and_registry(
        user, registry
    ).values("id")
    ...
    if filter_args.working_groups:
        ...
        filterable_wgs = [
            wg for wg in filter_args.working_groups if wg is not None
        ]

        query_filterable_wgs = Q(working_groups__id__in=filterable_wgs)

        if null_wgs:
            query_working_groups = query_filterable_wgs | Q(
                working_groups__id__isnull=True
            )
        else:
    ...
    if filter_args.consent_questions:
        for id in filter_args.consent_questions:
            patient_query = patient_query.filter(
                consents__answer=True, consents__consent_question__id=id
            )

    if filter_args.living_status:
        patient_query = patient_query.filter(
            living_status__in=filter_args.living_status
        )

    if (
        not user.is_superuser
        and filter_args.consent_checks
        and registry.has_feature(RegistryFeatures.CONSENT_CHECKS)
    ):
        consent_rules = ConsentRule.objects.filter(
            registry=registry,
            capability="see_patient",
            user_group__in=user.groups.all(),
            enabled=True,
        )
        for consent_question in [
            consent_rule.consent_question for consent_rule in consent_rules
        ]:
            patient_query = patient_query.filter(
                consents__answer=True,
                consents__consent_question=consent_question,
            )
...
def create_dynamic_all_patients_type(registry):
    ...
    def resolve_patients(
        parent: QueryResult, _info, id=None, sort=None, offset=None, limit=None
    ):
        def validate_sort_fields(sort_fields):
            sort_fields_without_order = [
                field.lstrip("-") for field in sort_fields
            ]
            return validate_fields(
                sort_fields_without_order, _valid_sort_fields, "sort field"
            )
    ...
    dynamic_query = type(
        f"DynamicAllPatients_{registry.code}",
        (graphene.ObjectType,),
        {
            "total": graphene.Int(),
            ...
            "data_summary": graphene.Field(
                create_dynamic_data_summary_type(registry)
            ),
            "resolve_data_summary": resolve_data_summary,
            "patients": graphene.List(
                create_dynamic_patient_type(registry),
                id=graphene.String(),
...
def create_dynamic_registry_type(registry):
    ...
    def resolve_all_patients(registry, _info, filter_args=PatientFilterType()):
        all_patients = list_patients_query(
            _info.context.user, registry, filter_args
        )
        ...
        if filter_args.search:
            for i, search_def in enumerate(filter_args.search):
                ...
                validate_fields(
                    search_def.fields, _valid_search_fields, "search field"
                )
                search_fields = [
                    to_snake_case(field) for field in search_def.fields
                ]

                search_annotations = {
                    f"sanitised_{i}_{field}": sanitise_search_field(field)
                    for field in search_fields
                }
                search_annotations.update(
                    {
                        f"search_{i}": (
                            SearchVector(
                                *search_annotations.keys(), config="simple"
                            )
                        )
                    }
                )

                all_patients = all_patients.annotate(
                    **search_annotations
                ).filter(**{f"search_{i}__icontains": search_text})

        return QueryResult(registry=registry, all_patients=all_patients)

    dynamic_registry_fields = {
        "all_patients": graphene.Field(
            create_dynamic_all_patients_type(registry),
            filter_args=graphene.Argument(PatientFilterType),
        ),
        "resolve_all_patients": resolve_all_patients,
    }
    return type(
        f"DynamicRegistryType_{registry.code}",
        (graphene.ObjectType,),
        dynamic_registry_fields,
    )
...
def create_dynamic_schema():
    ...
    for registry in Registry.objects.all():
        dynamic_query_fields.update(
            {
                registry.code: graphene.Field(
                    create_dynamic_registry_type(registry)
                ),
                f"resolve_{registry.code}": partial(
                    resolve_registry, registry=registry
                ),
            }
        )

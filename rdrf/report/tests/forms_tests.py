...
from report.forms import (
    get_demographic_field_value,
    get_demographic_field_choices,
    get_cde_choices,
    get_section_choices,
    get_working_group_field_value,
    get_working_group_choices,
    get_filter_consent_field_value,
...
class FormHelpersTestCase(TestCase):
    ...
    def test_get_demographic_field_value(self):
        self.assertEqual(
            ...
            get_demographic_field_value("patient", "dateOfBirth"),
        )
        self.assertEqual(
            '{"model": "patientaddressSet", "field": "addressType { type }"}',
            get_demographic_field_value(
                "patientaddressSet", "addressType { type }"
            ),
        )
    ...
    def test_get_demographic_field_choices(self):
        demographic_model = {
            ...
            "address": {
                ...
                "fields": {"street": "Street", "suburb": "Suburb"},
            },
        }
        ...
        self.assertEqual(
            [
                (
                    "Patient",
                    [('{"model": "patient", "field": "consent"}', "Consent")],
                ),
                (
                    "Patient Address",
                    [
                        ('{"model": "address", "field": "street"}', "Street"),
    ...
    def test_get_cde_field_value(self):
        ...
        self.assertEqual(
            '{"cfg": 1, "cde_key": "Form1__Section1__CDE1"}',
            get_cde_field_value(cfg1, "Form1__Section1__CDE1"),
        )
        self.assertEqual(
            '{"cfg": 2, "cde_key": "Sleep__SleepDiary__Time_To_Sleep"}',
            get_cde_field_value(cfg2, "Sleep__SleepDiary__Time_To_Sleep"),
        )
    ...
    def test_get_cde_choices(self):
        ...
        Section.objects.create(
            code="SleepDiary",
            display_name="Sleep Diary",
            elements="TimeToBed,TimeToWake",
        )
        ...
        form1 = RegistryForm.objects.create(
            ...
            name="SleepBehaviour",
            sections="SleepDiary",
            abbreviated_name="SleepBehaviour",
        )
        ...
        self.assertEqual(
            [
                ...
                (
                    '{"cfg": 1, "cde_key": "SleepBehaviour____SleepDiary____TimeToWake"}',
                    "Time Awoke",
                ),
            ],
            get_cde_choices(),
    ...
    def test_get_section_choices(self):
        ...
        self.assertEqual(
            [
                ("", "Show all Clinical Data Fields"),
                (
                    "Form1",
                    [
                        (
                            '{"registry": "reg1", "form": "Form1", "section": "Section A"}',
                            "Section A",
                        ),
                        (
                            '{"registry": "reg1", "form": "Form1", "section": "Section B"}',
                            "Section B",
                        ),
                    ],
                ),
                (
                    ...
                    [
                        (
                            '{"registry": "reg1", "form": "Form2", "section": "Section C"}',
                            "Section C",
                        ),
                        (
                            '{"registry": "reg1", "form": "Form2", "section": "Section D"}',
                            "Section D",
                        ),
                    ],
                ),
...
class WorkingGroupTestCase(TestCase):
    def setUp(self):
        ...
        self.wg1 = WorkingGroup.objects.create(
            id=1, registry=self.reg1, name="Working Group 1"
        )
        self.wg2 = WorkingGroup.objects.create(
            id=2, registry=self.reg2, name="Uncategorised"
        )

    def test_get_working_group_field_value(self):
        self.assertEqual(
            '{"registry": "reg1", "wg": 1}',
            get_working_group_field_value(self.wg1),
        )
        self.assertEqual(
            '{"registry": "reg2", "wg": 2}',
            get_working_group_field_value(self.wg2),
        )
...
class FilterConsentTestCase(TestCase):
    def setUp(self):
        ...
        self.section1 = ConsentSection.objects.create(
            registry=self.reg1, code="s1", section_label="Section 1"
        )
        self.section2 = ConsentSection.objects.create(
            registry=self.reg2, code="s2", section_label="Section 2"
        )
        self.cq1 = ConsentQuestion.objects.create(
            id=1, code="q1", section=self.section1, question_label="Question 1"
        )
        self.cq2 = ConsentQuestion.objects.create(
            id=2, code="q2", section=self.section2, question_label="Question 2"
        )

    def test_get_filter_consent_field_value(self):
        self.assertEqual(
            '{"registry": "abc", "consent_question": 1}',
            get_filter_consent_field_value(self.cq1),
        )
        self.assertEqual(
            '{"registry": "def", "consent_question": 2}',
            get_filter_consent_field_value(self.cq2),
        )

    def test_get_filter_consent_choices(self):
        self.assertEqual(
            [
                ('{"registry": "abc", "consent_question": 1}', "Question 1"),
...
class ReportDesignFormTestCase(TestCase):
    ...
    def test_duplicate_report_title_validation(self):
        ...
        ReportDesign.objects.create(
            title="Infancy Behaviours Report", registry=self.reg1
        )
        ...
        self.assertEqual(
            [
                'A report in this registry with the title "Sleep Report" already exists.'
            ],
            form_dup_title.errors["title"],
        )
    ...
    def test_clean_filter_working_groups(self):
        ...
        self.assertEqual(
            [wg1, wg2], form1.cleaned_data["filter_working_groups"]
        )
        self.assertEqual([wg3], form2.cleaned_data["filter_working_groups"])

    def test_clean_filter_consents(self):
        s1 = ConsentSection.objects.create(
            registry=self.reg1, section_label="Section 1", code="S1"
        )
        s2 = ConsentSection.objects.create(
            registry=self.reg2, section_label="Section 2", code="S2"
        )
        ...
        form1 = ReportDesignerForm(
            data=(
                dict(
                    ...
                    **{
                        "filter_consents": [
                            '{"registry": "reg1", "consent_question": 1}',
        ...
        form2 = ReportDesignerForm(
            data=(
                dict(
                    ...
                    **{
                        "filter_consents": [
                            '{"registry": "reg2", "consent_question": 3}'
                        ]
                    },
                )
            )
        ...
        form3 = ReportDesignerForm(
            data=(
                dict(
                    self.required_attrs,
                    **{
                        "filter_consents": [
                            '{"registry": "reg2", "consent_question": "NonExistent"}'
                        ]
                    },
                )
            )
    ...
    def test_save_to_model(self):
        # Given
        group_curator = Group.objects.create(
            name=RDRF_GROUPS.WORKING_GROUP_CURATOR
        )
        ...
        run_report_perm = Permission.objects.get(
            codename="can_run_reports",
        ...
        cs1 = ConsentSection.objects.create(
            registry=reg_ang, section_label="Section 1", code="S1"
        )
        cs2 = ConsentSection.objects.create(
            registry=reg_ang, section_label="Section 2", code="S2"
        )
        ...
        Section.objects.create(
            code="SleepDiary",
            elements="DayOfWeek,TimeToBed,TimeToSleep,NumTimesWoke,LongestTimeAwake",
        )
        form1 = RegistryForm.objects.create(
            name="SleepBehaviour",
            sections="SleepDiary",
        ...
        form = ReportDesignerForm(
            data={
                ...
                "filter_working_groups": [
                    f'{{"registry": "ang", "wg": {wg2_ang.pk}}}'
                ],
                "filter_by_consents": True,
                "filter_consents": [
                    '{"registry": "ang", "consent_question": 2}',
                    '{"registry": "ang", "consent_question": 3}',
                ],
                "demographic_fields": [
                    '{"model": "patient", "field": "givenNames"}',
                    '{"model": "patient", "field": "familyName"}',
                ],
                "cde_heading_format": ReportCdeHeadingFormat.ABBR_NAME.value,
                "cde_fields": [
                    '{"cfg": 1, "cde_key": "SleepBehaviour____SleepDiary____DayOfWeek"}',
                    '{"cfg": 1, "cde_key": "SleepBehaviour____SleepDiary____TimeToBed"}',
        ...
        self.assertEqual(
            "Reporting on general sleep trends", report_design.description
        )
        self.assertEqual(reg_ang, report_design.registry)
        self.assertEqual(
            [group_curator, group_clinician],
            list(report_design.access_groups.all()),
        )
        self.assertEqual(
            [wg2_ang], list(report_design.filter_working_groups.all())
        )
        self.assertEqual([cq2, cq3], list(report_design.filter_consents.all()))
        self.assertEqual(
            ReportCdeHeadingFormat.ABBR_NAME.value,
            report_design.cde_heading_format,
        )

        demographic_fields = report_design.reportdemographicfield_set.all()
        cde_fields = report_design.reportclinicaldatafield_set.all().order_by(
            "cde_key"
        )
        ...
        self.assertEqual(
            {"model": "patient", "field": "givenNames", "sort_order": 0},
            model_to_dict(
                demographic_fields[0], fields=("model", "field", "sort_order")
            ),
        )
        self.assertEqual(
            {"model": "patient", "field": "familyName", "sort_order": 1},
            model_to_dict(
                demographic_fields[1], fields=("model", "field", "sort_order")
            ),
        )

        self.assertEqual(
            {"cde_key": "SleepBehaviour____SleepDiary____DayOfWeek"},
            model_to_dict(cde_fields[0], fields=("cde_key",)),
        )
        self.assertEqual(
            {"cde_key": "SleepBehaviour____SleepDiary____TimeToBed"},
            model_to_dict(cde_fields[1], fields=("cde_key",)),
        )
        self.assertEqual(
            {"cde_key": "SleepBehaviour____SleepDiary____TimeToSleep"},
            model_to_dict(cde_fields[2], fields=("cde_key",)),
        )

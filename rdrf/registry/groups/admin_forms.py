...
class UserMixin:
    ...
    def clean_username(self):
        ...
        if (
            get_user_model()
            .objects.filter(username__iexact=username.lower())
            .exists()
        ):
    ...
    def clean(self):
        ...
        if len(registry_models) == 0:
            raise ValidationError(
                "Choosing a registry is mandatory if you selected a working group"
            )

        if len(working_group_models) == 0:
            raise ValidationError(
                "Choosing a working group is mandatory if you selected a registry"
            )

        for working_group_model in working_group_models:
            if working_group_model.registry not in registry_models:
                msg = (
                    "Working Group '%s' not in any of the assigned registries"
                    % working_group_model.display_name
                )
                raise ValidationError(msg)

        for registry_model in registry_models:
            if registry_model not in [
                working_group_model.registry
                for working_group_model in working_group_models
            ]:
                msg = (
                    "You have added the user into registry %s but not assigned the user "
        ...
        if user:
            if user.is_superuser:
                ...
                if self.instance.id:
                    ...
                    wg_queryset = WorkingGroup.objects.filter(
                        Q(id__in=instance_wgs) | Q(id__in=user_wgs)
                    )
                else:
...
class UserChangeForm(UserMixin, forms.ModelForm):
    ...
    password = ReadOnlyPasswordHashField(
        help_text=(
            OldUserChangeForm.base_fields["password"].help_text.format(
                "../password/"
            )
        )
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance:
            contains_clinician = any(
                RDRF_GROUPS.CLINICAL == g.name.lower()
    ...
    def clean_is_superuser(self):
        ...
        if is_superuser and not self.user.is_superuser:
            raise ValidationError(
                "Can't make user a superuser unless you are one!"
            )
        return is_superuser
    ...
    def clean_ethically_cleared(self):
        ...
        contains_clinician = any(
            RDRF_GROUPS.CLINICAL == g.lower() for g in group_names
        )
        if not contains_clinician and cleared:
            raise ValidationError(
                "You can enable ethical clearance only for clincians !"
            )
    ...
    def _validate_group(self, group_name):
        def group_error_msg(rdrf_group):
            ...
            return ValidationError(
                f"You can't assign this user to the {group_name} group because it isn't associated with a {group_member}"
            )

        if (
            RDRF_GROUPS.PARENT == group_name.lower()
            and not self.instance.parent_user_object.exists()
        ):

...
class PatientRegistration(BaseRegistration):
    def process(self, user):
        ...
        self.send_activation_email(
            registry_code, user, patient, self_registration=True
        )

    def send_activation_email(
        self, registry_code, user, patient, self_registration=True
    ):
        ...
        event_type = (
            ...
            else EventType.NEW_PATIENT_USER_ADDED
        )

        process_notification(registry_code, event_type, template_data)
        logger.info(
            f"Registration process - sent notification for {event_type}"
        )

    def update_django_user(self, django_user, registry):
        form_data = self.form.cleaned_data
        django_user.preferred_language = form_data.get(

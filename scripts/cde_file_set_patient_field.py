...
def main():
    ...
    for cde_file in cde_files:
        query_params = {"collection": "cdes", "django_model": "Patient"}
        cde_models = list(
            ClinicalData.objects.filter(**query_params)
            .order_by("-id")
            .values("django_id", "data")
        )
        patient_id = entry_exists(
            cde_models,
            cde_file.form_name,
            cde_file.section_code,
            cde_file.cde_code,
            cde_file.id,
        )
        ...
        if patient:
            logger.info(
                f"Set patient to {patient} for cde with id {cde_file.id}"
            )
            cde_file.patient = patient
            cde_file.save()
        else:
            logger.info(
                f"Could not determine patient for cde with id {cde_file.id}"
            )
